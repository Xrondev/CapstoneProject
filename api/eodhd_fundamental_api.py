...
def get_fundamental(symbol: str, exchange_id: str = 'US', save=True):
    ...
    url = f'https://eodhd.com/api/fundamentals/{symbol}.{exchange_id}?api_token={token}&fmt=csv'
    data = requests.get(url)

    if save:
        os.makedirs('../data/csv_eodhd/fundamental/', exist_ok=True)
        # csv save
        if data.status_code == 200:
            with open(f'../data/csv_eodhd/fundamental/fundamental_{symbol.replace(".", "_")}.csv',
                      'wb') as f:
